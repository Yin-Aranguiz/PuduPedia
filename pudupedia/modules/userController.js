...
const addAnimalSeenController = async (req, res) => {
    ...
    const { id: animalId } = req.body; // Recepción del id del animal
    try {
      const success = await addOrUpdateAnimalSeen(userId, animalId);
      res.json({ success });
    } catch (error) {
...
const addPlantSeenController = async (req, res) => {
    ...
    try {
      const result = await addOrUpdatePlantSeen(userId, plantId);
      res.json(result);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
...
const addParkVisitedController = async (req, res) => {
    ...
    try {
      const result = await addOrUpdateParkVisited(userId, parkId);
      res.json(result);
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
...
const updateAnimalSeenController = async (req, res) => {
  ...
  const { seen } = req.body; // Nuevos datos

  try {
    // Lógica para actualizar el registro en la base de datos
    const updatedAnimal = await updateAnimalSeen(id, seen);
...
const updatePlantSeenController = async (req, res) => {
  ...
  try {
    const updatedPlant = await updatePlantSeen(id, seen);
...
const updateParkVisitedController = async (req, res) => {
  ...
  try {
    const updatedPark = await updateParkVisited(id, visited);
